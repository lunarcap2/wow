...
$(function(){
  ...
  return false;   //<a>태그의 링크를 차단합니다. 
    ...
    // box2
      ...
      var mySlider3=$("#best_bg2 ul").bxSlider({
      mode:"horizontal",   //수평(horizontal) 방향으로 이동합니다.
        speed:500,   //이동 속도(500 : 0.5초)
        pager:false,   //페이징 표시를 제어(false:숨김, true:노출)합니다.
        moveSlides:1,   //이동 슬라이드 수입니다. 
        slideWidth:200,   //슬라이드 폭입니다. 
  ...
  $(".prev_btn2").on("click",function(){   //[이전]버튼을 눌렀을 때 이벤트 핸들러의 실행문을 실행
  mySlider3.goToPrevSlide();   //슬라이드를 한 단계 이전으로 이동합니다.
  return false;   //<a> 태그의 링크를 차단합니다.
  });

  $(".next_btn2").on("click",function(){   //[다음]버튼을 눌렀을 때 이벤트 핸들러의 실행문을 실행
  mySlider3.goToNextSlide();   //슬라이드를 한 단계 다음으로 이동합니다.
  return false;   //<a>태그의 링크를 차단합니다. 
  });
});  